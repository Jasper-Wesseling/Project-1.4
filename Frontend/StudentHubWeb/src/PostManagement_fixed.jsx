...
export default function PostManagement() {
    ...
    const fetchAllPosts = async (page = 1) => {
        ...
        try {
            ...
            // Backend returns simple array of posts (up to 20 per page)
            const allPosts = Array.isArray(data) ? data : [];
            const postsPerPage = 20; // Backend limit
            
            // Determine if there are more pages based on response length
            setHasMorePages(allPosts.length === postsPerPage);
            
            setPosts(allPosts);
            setTotalPosts(allPosts.length); // Current page posts count
            setCurrentPage(page);
            
            // Estimate total pages (we don't know exact total from backend)
            // This is a rough estimate - if we get full page, assume at least one more exists
            if (allPosts.length === postsPerPage) {
                setTotalPages(page + 1); // At least current + 1 more
            } else {
                setTotalPages(page); // This is likely the last page
            }
        }
        catch (error) {
    ...
    const fetchPostsByUser = async (email, page = 1) => {
        ...
        try {
            ...
            // Backend returns simple array, handle pagination like Products.jsx
            const allPosts = Array.isArray(data) ? data : [];
    ...
    const renderPagination = () => {
        // Show pagination if we have posts and either multiple pages or potential for more
        if (totalPosts === 0 || (currentPage === 1 && !hasMorePages)) return null;
            ...
            <div className="pagination">
                ...
                {hasMorePages && currentPage === totalPages - 1 && (
                    <>
                        ...
                        <button onClick={() => handlePageChange(currentPage + 1)} className="page-btn">
                            {currentPage + 1}
                        </button>
                    </>
                )}
                    ...
                    <div className="posts-container">
                        <div className="results-header">
                            ...
                            <span className="results-count">
                                {totalPosts} post{totalPosts !== 1 ? 's' : ''} on this page
                                {hasMorePages ? ` (Page ${currentPage}, more available)` : ` (Page ${currentPage})`}
                            </span>
                        </div>
